...
class ActionResendRegisterCode
{
    ...
    {
        if (empty($data->phone)) {
            throw new BadRequestHttpException($this->translator->trans('user.registration.errors.phone_required'));
        }
        ...
        $code = $this->codeService->createCodeWithPhone($user, $user->phone, Code::TYPE_ACTIVATE);

        $code->codeType = Code::TYPE_ACTIVATE;
